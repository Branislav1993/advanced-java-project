package rs.fon.parlament.config;
...
public class Settings {
	...
	private static String absoluteConfigPath = System.getenv("OPEN_PARLIAMENT") + File.separator + ".parlament_crud"
			+ File.separator;
	...
	private void loadConfig() throws Exception {
		...
		if (System.getenv("OPEN_PARLIAMENT") != null) {
			absoluteConfigPath = System.getenv("OPEN_PARLIAMENT") + File.separator + ".parlament_crud" + File.separator;
			logger.info("Used OPEN_PARLIAMENT variable.");
		} else {
			absoluteConfigPath = System.getProperty("user.home") + File.separator + ".parlament_crud" + File.separator;
		...
		try {
			...
			// if there is a config file in the /.parlament_crud folder
			// load that one
			...
			// otherwise, load the default config file
				...
				loadDefaultConfig();
				if (config != null) {
					// and save it to the <USER_HOME>/.parlament_crud folder
					saveConfig();
				} else {
					throw new Exception();
				}
